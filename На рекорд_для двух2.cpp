...
void Stolknov_s_Prep (COLORREF color, const char* text1, int x, int y, int* vx, int* vy, int* vx1, int* vy1, int Xpr, int Ypr, int r);
...
void Ottalkiv (int x1, int y1, int x2, int y2, int r1, int r2, int* vx1, int* vy1, int* vx2, int* vy2);
...
void MoveShar ()
    ...
    while (!GetAsyncKeyState(VK_ESCAPE))
        ...
        Stolknov_s_Prep (TX_LIGHTGREEN, "Win 1!", x, y, &vx, &vy, &vx1, &vy1, Xpr, Ypr, r1);
        Stolknov_s_Prep (TX_LIGHTGREEN, "Win 1!", x, y, &vx, &vy, &vx1, &vy1, Xpr1, Ypr1, r1);
        Stolknov_s_Prep (TX_LIGHTGREEN, "Win 1!", x, y, &vx, &vy, &vx1, &vy1, Xpr2, Ypr2, r1);
        Stolknov_s_Prep (TX_LIGHTGREEN, "Win 1!", x, y, &vx, &vy, &vx1, &vy1, Xpr3, Ypr3, r1);
        Stolknov_s_Prep (TX_LIGHTGREEN, "Win 1!", x, y, &vx, &vy, &vx1, &vy1, Xpr4, Ypr4, r1);

        Stolknov_s_Prep (TX_LIGHTBLUE, "Win 2!", x1, y1, &vx1, &vy1, &vx, &vy, Xpr, Ypr, r2);
        Stolknov_s_Prep (TX_LIGHTBLUE, "Win 2!", x1, y1, &vx1, &vy1, &vx, &vy, Xpr1, Ypr1, r2);
        Stolknov_s_Prep (TX_LIGHTBLUE, "Win 2!", x1, y1, &vx1, &vy1, &vx, &vy, Xpr2, Ypr2, r2);
        Stolknov_s_Prep (TX_LIGHTBLUE, "Win 2!", x1, y1, &vx1, &vy1, &vx, &vy, Xpr3, Ypr3, r2);
        Stolknov_s_Prep (TX_LIGHTBLUE, "Win 2!", x1, y1, &vx1, &vy1, &vx, &vy, Xpr4, Ypr4, r2);
        ...
        Ottalkiv (x, y, x1, y1, r1, r2, &vx, &vy, &vx1, &vy1);

        if (Dist (x2, y2, x1, y1) <= r2 + r2)
            {
            txSetFillColor (TX_BLACK);
            txClear ();
...
void upravlenie (int* vx, int* vy, double* v, COLORREF vverh, COLORREF vlevo, COLORREF vniz, COLORREF vpravo)
    {
    if (GetAsyncKeyState (vverh && vlevo))
        {
        *vy = -*v;
        *vx = -*v;
        }
    if (GetAsyncKeyState (vverh && vpravo))
        {
        *vy = -*v;
        *vx = *v;
        }
    if (GetAsyncKeyState (vniz && vlevo))
        {
        *vy = *v;
        *vx = -*v;
        }
    if (GetAsyncKeyState (vniz && vpravo))
        {
        *vx = *v;
        *vy = *v;
        }
